...
func TestSharedMaintenance_ProductionTaskRosterQuietWindow(t *testing.T) {
	// #53: named production DB-touching background task kinds share one window.
	// Cycles record "DB hits"; no real PostgreSQL. Roster mirrors current MD
	// adaptive loops so phase-skew cannot reappear when new tasks forget the gate.
	roster := []string{
		"deposit",
		"company-fund-provider-event",
		"company-fund-reconciliation",
		"company-fund-account-registry",
		"company-fund-safeheron-coin-catalog",
		"company-fund-safeheron-collector",
		"company-fund-current-rate-refresh",
		"company-fund-valuation-sweep",
		"fund-routing-worker",
		"fund-routing-projection",
		"fund-routing-alert-notifier",
		"fund-routing-alert-escalator",
		"fund-routing-alert-reconciler",
		"fund-routing-metrics",
		"wallet-registry-refresh",
		"address-pool-replenisher",
		"address-scheduler",
	}

	maxIdle := 120 * time.Millisecond
	neonThreshold := 50 * time.Millisecond
	window := adaptiveschedule.NewMaintenanceWindow(maxIdle)
	window.SetOpenFor(12 * time.Millisecond)
	...
	times := append([]time.Time(nil), hits...)
	mu.Unlock()
	if len(times) < len(roster) {
		t.Fatalf("expected each roster task to hit at least once on startup; hits=%d roster=%d", len(times), len(roster))
	}
	...
	if maxGap < neonThreshold {
		t.Fatalf("production roster quiet gap %s < neon threshold %s (hits=%d)", maxGap, neonThreshold, len(times))
	}
	if maxGap < maxIdle/2 {
		t.Fatalf("production roster quiet gap %s should approach MaxIdle=%s", maxGap, maxIdle)
	}
}
