...
func (r *DBRepository) loadChains(ctx context.Context) ([]*Chain, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT code, name, network_family, COALESCE(chain_id, ''), native_symbol, COALESCE(explorer_url, ''), COALESCE(short_name, ''), enabled, display_order
	...
	for rows.Next() {
		c := &Chain{}
		if err := rows.Scan(&c.Code, &c.Name, &c.NetworkFamily, &c.ChainID, &c.NativeSymbol, &c.ExplorerURL, &c.ShortName, &c.Enabled, &c.DisplayOrder); err != nil {
			return nil, err
		}
...
func (r *DBRepository) loadCoinChains(ctx context.Context) ([]*CoinChain, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, chain_code, coin_id, is_native, COALESCE(token_contract, ''), decimals,
		        ...
		        required_confirmations, COALESCE(token_standard, ''), COALESCE(estimated_arrival_minutes, 0),
		        display_order
	...
	for rows.Next() {
		...
		if err := rows.Scan(&cc.ID, &cc.ChainCode, &cc.CoinID, &cc.IsNative, &cc.TokenContract,
			...
			&cc.WithdrawEnabled, &cc.RequiredConfirmations, &cc.TokenStandard,
			&cc.EstimatedArrivalMinutes, &cc.DisplayOrder); err != nil {
			return nil, err
		}
		ccs = append(ccs, cc)
	}
