...
func (r *Registry) AllEnabledCoinChains() []*CoinChain {
	r.mu.RLock()
	defer r.mu.RUnlock()
	var result []*CoinChain
	for _, ccs := range r.byChain {
		result = append(result, ccs...)
	}
...
func (r *Registry) AllCoins() []*Coin {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result := make([]*Coin, 0, len(r.coins))
